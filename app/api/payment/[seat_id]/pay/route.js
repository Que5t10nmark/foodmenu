...
export async function PUT(req, context) {
  ...
  const { discount = 0, method, printReceipt = false } = await req.json();

  if (!seat_id || !method) {
    return new Response(JSON.stringify({ message: "กรุณาระบุ seat_id และ method" }), { status: 400 });
  }

  try {
    const [orders] = await db.execute(
      `SELECT * FROM purchase
       WHERE seat_id = ? AND purchase_status = 'เสร็จแล้ว'
    ...
    for (const order of orders) {
      await db.execute(
        `INSERT INTO payment_detail (payment_id, purchase_id)
         VALUES (?, ?)`,
